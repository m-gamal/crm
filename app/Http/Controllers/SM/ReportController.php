...
class ReportController extends Controller
{
    ...
    public function singleYours($id)
    {
        ...
        $singleReport['promotedProducts']   =   SMReportPromotedProduct::select('product_id')
                                                                        ->where('report_id', $singleReport->id)
                                                                        ->get();

        $singleReport['sampleProducts']     =   SMReportSampleProduct::select('product_id')
                                                                    ->where('report_id', $singleReport->id)
                                                                    ->get();

        $singleReport['gifts']              =   SMReportGift::select('gift_id')
                                                            ->where('report_id', $singleReport->id)
                                                            ->get();

        $singleReport['soldProducts']       =   SMReportSoldProduct::select('product_id', 'quantity')
                                                                    ->where('report_id', $singleReport->id)
                                                                    ->get();
    ...
    public function listAllPendingServicesRequests()
    {
        ...
        $pendingServicesRequests =  AMServiceRequest::pending()->whereIn('am_id', $AMsId)->get();
    ...
    public function doCreateExpenseReport(CreateExpenseReportRequest $request)
    {
        ...
        $expenseReport->sm_id               = \Auth::user()->id;
        ...
        try {
            if ($expenseReport->save()) {
                ...
                ->move(public_path('uploads/expenses_reports/'. \Auth::user()->id .'/'. $expenseReport->month . '/'), $expenseReport->date . '.' . $extension);
            }
    ...
    {
        $expensesReports =  SMExpenseReport::where('sm_id', \Auth::user()->id)->get();
        $dataView 	= [
    ...
    {
        $servicesRequests = SMServiceRequest::where('sm_id', \Auth::user()->id)->get();
        $dataView 	= [
    ...
    public function doCreateServiceRequest(CreateServiceRequestRequest $request)
    {
        ...
        $serviceRequest->sm_id          = \Auth::user()->id;
